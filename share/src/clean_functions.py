...
def clean_data(df, log, skip_cols=None, clean_dict=None, types_dict=None,
               use_middle_names=False, allow_name_errors=False):
    ...
    if types_dict is None:
        with open('hand/column_types.yaml', 'r') as file:
            types_dict = yaml.load(file, Loader=yaml.FullLoader)
    ...
    for col_name in df.columns.values:
        if col_name in skip_cols or col_name not in types_dict.keys():
        ...
        elif types_dict[col_name] == 'rank':
            log.info("Column '%s' cleaned using RankCleaners." % col_name)
            cleaned_df[col_name] = RankCleaners(df[col_name], log).clean()
            ...
            cleaned_df[cleaned_dt_df.columns] = cleaned_dt_df
