...
class NameCleaners:
    def __init__(self, full_name='', human_name='',
                 ...
                 middle_initial='', middle_name='', allow_name_errors=False):
        ...
        self.middle_name = string_strip(middle_name, no_sep=False)
        self.middle_initial = string_strip(middle_initial, no_sep=True)
        self.middle_initial2 = ''

        self.suffix_name = ''
        self.second_name = ''
        self.allow_name_errors = allow_name_errors
        if full_name:
    ...
    def extract_suffix_name(self, name, suffixes):
        ...
        if not self.allow_name_errors:
            assert len(suffix) < 2, 'Too many suffixes found {} in {}'.format(suffix, name)
        else:
            if len(suffix) > 1:
                suffix = ["-".join(suffix)]
    ...
    def extract_parts(self):
        ...
        if mi_parts:
            if not self.allow_name_errors:
                assert len(mi_parts) <= 2, print(self.__dict__)
            else:
                if len(mi_parts) > 2:
                    mi_parts[1:] = "-".join(mi_parts[1:])
        ...
        if sn_parts:
            if not self.allow_name_errors:
                assert len(sn_parts) <= 1, print(self.__dict__)
            else:
                if len(sn_parts) > 1:
                    sn_parts = ["-".join(sn_parts)]
