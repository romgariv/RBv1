...
class GeneralCleaners:
    ...
    def clean_from_yaml(self, fill=np.nan):
        ...
        with open('hand/{}_types.yaml'.format(self.col_type), 'r') as file:
            type_dict = yaml.load(file, Loader=yaml.FullLoader)
        cleaned_col = self.col.str.upper()\
            .replace(r"^\s+|\s+$|\s+(?=\s)", "", regex=True)\
            .replace(type_dict)
    ...
    def clean_money(self, money, na_value=np.nan):
        """Cleans money
        Parameters
        ----------
        money : str
        ...
        Returns
        -------
        cleaned_money : float, np.nan
        """
        if isinstance(money, str):
            try:
                cleaned_money = float(Decimal(re.sub(r'[^\d\-.]', '', money)))
            except:
                cleaned_money = na_value
        elif np.isfinite(money):
            cleaned_money = float(money)
        else:
            cleaned_money = na_value
...
class DateTimeCleaners:
    ...
    def clean(self):
        ...
        if self.dt_type == 'time':
            ...
            if np.issubdtype(self.dt_col.dtype, np.datetime64):
                date_col = self.dt_col.dt.date
                cleaned_time_col = self.dt_col.dt.time

            else:
                date_col = self.dt_col\
                    .fillna('')\
                    .map(lambda x: x.split(' ', 1)[0])
                time_col = self.dt_col\
                    .fillna('')\
                    .map(lambda x: x.split(' ',1)[-1])
                cleaned_time_col = self.clean_times(time_col)

            cleaned_df[date_name] = self.clean_dates(date_col)
            cleaned_df[time_name] = cleaned_time_col
        return cleaned_df
    ...
    def correct_dates(self, date_col):
        ...
        """
        today = pd.to_datetime('2019-12-31').date()
...
class RankCleaners:
    RANK_EXPORT_PATHS = ['../hand/rank_types.csv',
                         '../../../share/hand/rank_types.csv']
    def __init__(self, rank_col, log):
        col = rank_col\
            .fillna('')\
        ...
        REP = None
        for rep in RANK_EXPORT_PATHS:
            if os.path.exists(rep):
                REP = rep
                break
        assert not REP is None, 'Need rank export path'
        rank_df = pd.read_csv(REP)
        missing_ranks = col[~col.isin(rank_df['known_rank'])]
        log.warning('rank_types file missing %d ranks. Please fill in.' % missing_ranks.nunique())
        cleaned_ranks = col.to_frame().rename(columns={0: 'known_rank'})
        rank_dict = dict(zip(rank_df['known_rank'], rank_df['cleaned_rank']))
        is_officer_dict = dict(zip(rank_df['known_rank'], rank_df['is_sworn_officer']))

        rank_df = rank_df.merge(cleaned_ranks.drop_duplicates(),
                                how='outer', on='known_rank')
        rank_df.sort_values('cleaned_ranks').to_csv(REP, index=False)

        cleaned_ranks[rank_col] = cleaned_ranks['known_rank'].replace(rank_dict)
        cleaned_ranks['is_sworn_officer'] = cleaned_ranks['known_rank'].replace(is_officer_dict)
        self.cleaned_ranks = cleaned_ranks

    def clean(self):
        return self.cleaned_ranks
